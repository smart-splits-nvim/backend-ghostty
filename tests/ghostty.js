...
function run(argv) {
  ...
  var pid = argv[1] === "owner" ? owningTerminal().pid : Number(argv[1]);
  ...
  function configuration(command) {
    ...
    fields.setDescriptorForKeyword(
      D.descriptorWithString(command),
      code("GScC"),
    );
  ...
  if (operation === "new") {
    var win = send(
      ...
      "Ghst",
      "NWin",
      argv[3] ? { GNwS: configuration(argv[3]) } : {},
    );
    return ObjC.unwrap(
      send(pid, "core", "getd", {
  ...
  var term = terminal("Gtrm", argv[3]);
  if (operation === "action") {
    return !!send(pid, "Ghst", "PfAc", {
      "----": D.descriptorWithString(argv[4]),
      GonT: term,
    }).booleanValue;
  }
  if (operation === "split") {
    ...
    var params = {
      "----": term,
      GSpd: D.descriptorWithEnumCode(code(direction)),
    };
    if (argv[5]) params.GSpS = configuration(argv[5]);
    var created = send(pid, "Ghst", "Splt", params);
    return ObjC.unwrap(
      send(pid, "core", "getd", { "----": prop("ID  ", created) }).stringValue,
    );
  }
  if (operation === "focus") {
    send(pid, "Ghst", "Fcus", { "----": term });
    return;
  }
  if (operation === "close") {
    send(pid, "Ghst", "Clos", { "----": term });
    return;
  }
  if (operation === "text") {
    send(pid, "Ghst", "InTx", {
  ...
  if (operation === "key") {
    var params = {
      "----": D.descriptorWithString(argv[4]),
      GKeM: D.descriptorWithString(argv[5]),
      GKeT: term,
    };
